...
def main() -> None:
    ...
    doc.add_paragraph(
        "We evaluate forecast performance using Mean Error (ME), Mean Absolute Error (MAE), Root Mean Squared Error (RMSE), and Mean Absolute Percentage Error (MAPE). "
        "ME is especially important because it captures the direction of bias: a positive ME indicates systematic overprediction, while a negative ME indicates underprediction. "
        "As a benchmark, we use a random-walk forecast equal to the most recently completed annual Brent price available at the time of release."
    )
    doc.add_paragraph(
    ...
    add_metric_table(
        ...
        benchmark[["model", "horizon_years", "n_forecasts", "ME", "MAE", "RMSE", "MAPE"]],
    )
